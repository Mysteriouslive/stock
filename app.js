...
let AUTO_REFRESH_INTERVAL =
    Number(localStorage.getItem('stockRefreshRate')) || 10000;
...
async function fetchTwseName(code) {
    try {
        const cleanCode = displaySymbol(code);
        const res = await fetch(
            `${WORKER_URL}/?source=twse_name&symbol=${encodeURIComponent(cleanCode)}`,
            { cache: 'no-store' }
        );
...
async function fetchTwseMetrics(symbol) {
    try {
        ...
        const res = await fetch(
            `${WORKER_URL}/?source=twse_metrics&symbol=${encodeURIComponent(code)}`,
            { cache: 'no-store' }
        );
        if (!res.ok) return null;
...
async function fetchForexData() {
    try {
        ...
        return {
            rate,
            changePercent: Number(data.changePercent ?? data.change_percent ?? data.percentChange ?? NaN),
...
function resetFundamentals() {
    ...
    const subtitle = document.getElementById('fundamentals-subtitle');
    if (subtitle) subtitle.textContent = 'Finnhub · Fundamental Metrics';
...
function renderTwseMetrics(twseData, symbol, quoteResult, currentPrice) {
    ...
    const marketName = isOtc ? '櫃買中心 (TPEx)' : '證交所 (TWSE)';

    const subtitle = document.getElementById('fundamentals-subtitle');
    if (subtitle) {
        subtitle.textContent = `台灣在地市場 · ${twseData?.source || marketName}`;
    }
    ...
    if (!high52 && Array.isArray(quotes.high)) {
        const values = quotes.high.filter(v => v != null && Number.isFinite(Number(v)));
        if (values.length) high52 = Math.max(...values);
    }

    if (!low52 && Array.isArray(quotes.low)) {
        const values = quotes.low.filter(v => v != null && Number.isFinite(Number(v)));
        if (values.length) low52 = Math.min(...values);
    }
    ...
    setMetric('fundamentals-note', `資料來源：${twseData?.source || marketName} + Yahoo Finance · 代碼 ${displaySymbol(symbol)}`);
    setMetric('profile-country', '台灣');
    setMetric('profile-industry', meta.fullExchangeName || (isOtc ? '上櫃股票' : '上市股票'));
    setMetric('profile-currency', meta.currency || 'TWD');

    const desc = document.getElementById('profile-description');
    if (desc) {
        desc.textContent = `${meta.longName || displaySymbol(symbol)} - 台灣在地公開資訊（包含本益比、殖利率、股價淨值比，EPS 由股價與本益比推算）。`;
        desc.classList.add('hidden');
    }
...
let watchlist = JSON.parse(localStorage.getItem('stockWatchlist') || 'null') || [...DEFAULT_STOCKS];
...
function renderWatchlist() {
    ...
    sorted.forEach(stock => {
        ...
        btn.className = `stock-btn w-full text-left px-3 py-2.5 rounded-xl text-gray-400 flex items-center gap-2 group ${
            currentSymbol === stock.symbol ? 'bg-white/10 text-white' : ''
        }`;
        btn.dataset.symbol = stock.symbol;
        btn.onclick = () => loadStock(stock.symbol);

        const priceText = quote?.latestPrice
            ? `<span class="text-[10px] text-gray-400">${quote.latestPrice}</span>`
            : '';

        const changeNumber = Number(quote?.changePercent);
        const changeText = quote?.changePercent != null && Number.isFinite(changeNumber)
            ? `<span class="text-[10px] ${changeNumber >= 0 ? 'price-up' : 'price-down'}">${changeNumber >= 0 ? '+' : ''}${changeNumber.toFixed(2)}%</span>`
            : '';

        btn.innerHTML = `
            <span class="drag-handle text-lg px-2 py-1 ${
                sortMode === 'manual' ? 'cursor-grab hover:text-white' : 'opacity-30'
            }" title="${sortMode === 'manual' ? '拖曳排序' : '切換到自訂順序後可拖曳'}">⋮</span>
            <span class="w-8 h-8 rounded-lg ${colorClass} flex items-center justify-center text-[10px] font-bold shrink-0">
                ${displaySymbol(stock.symbol).slice(0, 4)}
            </span>
            <div class="flex-1 min-w-0">
                <div class="font-medium text-sm truncate">
                    ${stock.name || displaySymbol(stock.symbol)}
                </div>
                <div class="text-[11px] text-gray-500 flex items-center gap-2">
                    <span>${displaySymbol(stock.symbol)}</span>
                    ${priceText}
                    ${changeText}
                </div>
            </div>
            <button class="delete-btn text-gray-600 hover:text-red-400 p-1 rounded-lg hover:bg-red-500/10"
                onclick="event.stopPropagation(); removeStockBySymbol('${escapeHtmlAttribute(stock.symbol)}')" title="移除">
...
async function addStock() {
    ...
    if (isForexSymbol(raw)) {
        ...
        try {
            ...
            try {
                const twoRes = await fetchYahooData(`${raw}.TWO`, '1d', '5d');
                if (twoRes) symbol = `${raw}.TWO`;
            } catch {
                symbol = `${raw}.TW`;
            }
        }
    ...
    if (watchlist.some(s => s.symbol === symbol || displaySymbol(s.symbol) === displaySymbol(symbol))) {
        alert(`${displaySymbol(symbol)} 已經在清單中了`);
        input.value = '';
        return;
    }
    ...
    if (isForexSymbol(symbol)) {
        ...
        const cryptoNames = { BTC: '比特幣', ETH: '以太幣', SOL: 'Solana', XRP: 'XRP', ADA: 'Cardano', DOGE: 'Dogecoin', BNB: 'BNB' };
...
function removeStock(index) {
    ...
    if (currentSymbol === removed.symbol) {
        ...
        if (currentSymbol) {
            ...
            setText('stock-symbol-title', '—');
            setText('stock-name', '選擇或新增股票以查看詳情');
...
// ============================================================
// Yahoo Finance (核心探測器：自動在 .TW 與 .TWO 之間切換)
// ============================================================
...
function parseQuote(result, symbol = '') {
    ...
    const price = meta.regularMarketPrice ?? lastClose ?? previousCloseValue;
...
function parseCandles(result) {
    ...
    for (let i = 0; i < timestamps.length; i++) {
        ...
        if ([open, high, low, close].some(v => v == null || !Number.isFinite(Number(v)))) {
            continue;
        }

        let time;
        if (['1d', '1wk', '1mo'].includes(currentPeriod.interval)) {
...
function applyCurrentPriceToQuote(quote, currentPrice, previousClose, symbol, fallbackChangePercent = NaN) {
    ...
    if (Number.isFinite(previous) && previous > 0) {
        ...
        const estimatedPrevious = current / (1 + percent / 100);
        if (Number.isFinite(estimatedPrevious) && estimatedPrevious > 0) {
            change = current - estimatedPrevious;
            quote.previousClose = formatPrice(estimatedPrevious, symbol);
        }
    }
...
function renderCompanyInfo(result, symbol, quote, source = 'Yahoo Finance') {
    ...
    if (isForexSymbol(symbol)) {
        ...
        const nameMap = { BTC: '比特幣 (Bitcoin)', ETH: '以太幣 (Ethereum)', SOL: 'Solana', XRP: 'XRP', ADA: 'Cardano', DOGE: 'Dogecoin', BNB: 'BNB' };
    ...
    if (note) {
        note.textContent = `資料來源：${source} · ${meta.fullExchangeName || meta.exchangeName || '交易市場'} · 代碼 ${displaySymbol(symbol)}`;
    }
}
...
function renderFinnhubCompanyInfo(profile, symbol, quote) {
    ...
    if (note) {
        const extra = [
            p.country ? `國家 ${p.country}` : '',
            p.marketCapitalization ? `市值 ${Number(p.marketCapitalization).toLocaleString('zh-TW')} 百萬` : '',
            p.ipo ? `IPO ${p.ipo}` : ''
        ].filter(Boolean).join(' · ');
...
function initChart() {
    ...
    if (container.clientHeight <= 0) {
        container.style.minHeight = window.innerWidth < 768 ? '330px' : '400px';
    }
...
async function loadStock(symbol, isSilent = false) {
    if (!symbol) return;
    if (isLoadingStock && !isSilent) return;
    ...
    if (!isSilent) {
        ...
        if (priceChange) priceChange.className = 'text-base md:text-lg font-semibold px-2.5 py-1 rounded-lg bg-white/5';

        const currentPrice = document.getElementById('current-price');
        if (currentPrice) currentPrice.className = 'text-4xl md:text-5xl font-extrabold text-white tracking-tight';
    ...
    try {
        ...
        if (isForexSymbol(symbol)) {
            try {
                ...
                if (forex?.rate) {
                    ...
                    if (!isSilent) {
                        renderCompanyInfo(yahooResult, symbol, quote, yahooResult ? 'Frankfurter 現價 + Yahoo Finance K線' : 'Frankfurter');
                    }
                }
            } catch {}
        ...
        // 3. 加密貨幣處理
            ...
            try {
                ...
                const cryptoSymbol = displaySymbol(symbol).toUpperCase();
                const crypto = cryptoJson?.[cryptoSymbol];

                if (crypto) {
                    const currentPrice = Number(crypto.usd ?? crypto.price);
                    const cryptoChange = Number(crypto.usd_24h_change ?? crypto.changePercent ?? crypto.change_24h ?? NaN);
                    ...
                    if (!isSilent) {
                        renderCompanyInfo(yahooResult, symbol, quote, yahooResult ? 'CoinGecko 現價 + Yahoo Finance K線' : 'CoinGecko');
                    }
                }
            } catch {}
            if (!quote && quoteCache[symbol]) quote = quoteCache[symbol];
        ...
        // 渲染報價
        setText('current-price', quote.latestPrice);
        setText('open-price', quote.open);
        setText('high-price', quote.high);
        setText('low-price', quote.low);
        setText('previous-close', quote.previousClose);
        setText('volume', quote.volume);

        const changeEl = document.getElementById('price-change');
        const changeNumber = Number(quote.change);
        const changePercent = Number(quote.changePercent);
        const isUp = Number.isFinite(changeNumber) && changeNumber > 0;
        const isDown = Number.isFinite(changeNumber) && changeNumber < 0;
        const isFlat = !isUp && !isDown;
        const sign = isUp ? '+' : '';

        if (changeEl) {
            changeEl.textContent = `${sign}${quote.change} (${sign}${Number.isFinite(changePercent) ? changePercent.toFixed(2) : '0.00'}%)`;
        }

        const currentPriceEl = document.getElementById('current-price');
        if (isFlat) {
            if (changeEl) changeEl.className = 'text-base md:text-lg font-semibold px-2.5 py-1 rounded-lg bg-white/5 text-gray-400';
            if (currentPriceEl) currentPriceEl.className = 'text-4xl md:text-5xl font-extrabold tracking-tight text-gray-300';
        } else if (isUp) {
            if (changeEl) changeEl.className = 'text-base md:text-lg font-semibold px-2.5 py-1 rounded-lg bg-white/5 price-up';
            if (currentPriceEl) currentPriceEl.className = 'text-4xl md:text-5xl font-extrabold tracking-tight price-up';
        } else {
            if (changeEl) changeEl.className = 'text-base md:text-lg font-semibold px-2.5 py-1 rounded-lg bg-white/5 price-down';
            if (currentPriceEl) currentPriceEl.className = 'text-4xl md:text-5xl font-extrabold tracking-tight price-down';
        }
        ...
        if (chart && candlestickSeries && chartData.length > 0) {
            ...
            if (candlestickSeries && currentChartData.length > 0) {
                ...
                setText('chart-status', `${currentPeriod.label} · 保留上一筆 K 線`);
            } else {
                setText('chart-status', yahooError ? 'K線資料暫時無法取得' : '目前週期沒有 K 線資料');
            }
        }

        const now = new Date();
        setText('last-update', `最後更新：${now.toLocaleString('zh-TW', { hour12: false })}`);
        renderWatchlist();

    } catch (error) {
        console.error('loadStock error:', error);
        if (!isSilent) {
            setText('chart-status', '資料取得失敗');
            setText('stock-name', `找不到 ${displaySymbol(symbol)} 的資料，請確認代碼`);
            setText('price-change', '載入失敗');
        }
    } finally {
...
function restartAutoRefresh() {
    stopAutoRefresh();
    if (currentSymbol) startAutoRefresh();
}
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (currentSymbol && watchlist.some(stock => stock.symbol === currentSymbol)) {
        await loadStock(currentSymbol);
    } else if (watchlist.length > 0) {
        currentSymbol = watchlist[0].symbol;
        await loadStock(currentSymbol);
    }
...
function closeModals() {
    ...
    if (settingsModal) settingsModal.classList.add('opacity-0', 'scale-95');
    if (profileModal) profileModal.classList.add('opacity-0', 'scale-95');
...
function loadUserPreferences() {
    ...
    if (colorSelect) {
        ...
        colorSelect.onchange = applySettings;
    }

    if (refreshSelect) {
        refreshSelect.value = savedRefreshRate;
        refreshSelect.onchange = applySettings;
    }
