...
const chart = LightweightCharts.createChart(chartContainer, {
    ...
    timeScale: { 
        ...
        tickMarkFormatter: (time, tickMarkType, locale) => {
            ...
            if (currentPeriod.interval === '1d') return `${month}月${day}日`;
...
function updateChartColors(mode) {
    if (candlestickSeries) {
        if (mode === 'green-red') {
            candlestickSeries.applyOptions({
                borderVisible: true, upColor: '#10b981', downColor: '#ef4444', 
    ...
    if (mode === 'green-red') {
        styleTag.innerHTML = `.price-up { color: #10b981 !important; } .price-down { color: #ef4444 !important; }`;
    } else {
        styleTag.innerHTML = `.price-up { color: #ef4444 !important; } .price-down { color: #10b981 !important; }`;
    }
    renderWatchlist();
}
